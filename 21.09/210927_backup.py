...
# import cv2
#
# img_src = cv2.imread('images (1)/tomato.jpg', cv2.IMREAD_COLOR)
#
# # 이미지를 BGR에서 HSV로 색변환
...
# upper_yellow = (40,255,255) # 자료형은 튜플형태로(H,S,V)
#
# lower_white = (150,150,150) # 자료형은 튜플형태로(H,S,V)
# upper_white = (255,255,255) # 자료형은 튜플형태로(H,S,V)
#
# img_mask_w = cv2.inRange(img_src, lower_white, upper_white) # 초록 정보 추출
# img_dst_w = cv2.bitwise_and(img_src, img_src, mask=img_mask_w)
#
# img_mask_y = cv2.inRange(img_hsv, lower_yellow, upper_yellow) # 초록 정보 추출
# img_dst_y = cv2.bitwise_and(img_src, img_src, mask=img_mask_y)
#
# cv2.imshow('white',img_dst_w)
# cv2.imshow('yellow',img_dst_y)
#
# cv2.waitKey(0)
# cv2.destroyAllWindows()
