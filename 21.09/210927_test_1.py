...
while True:
    ...
    # 선 그리기
    for i, line in enumerate(lines):
        cv2.line(img_dst, (line[0][0], line[0][1]),
    ...
    img_dst = cv2.polylines(img_dst, [pts], True, (255, 0, 255), 3)
    ## 추출 영역 표시

    ## 사이즈
    _, img_src = imgsize(img_src, 1000)
    _, img_bit_w = imgsize(img_bit_w, 1000)
    _, img_bit_y = imgsize(img_bit_y, 1000)
    _, img_bit_w_b = imgsize(img_bit_w_b, 1000)
    _, img_bit_y_b = imgsize(img_bit_y_b, 1000)
    ...
    a = cv2.hconcat([img_bit_w,img_bit_y])
    b = cv2.hconcat([img_bit_w_b, img_bit_y_b])

    cv2.imshow('draw_lines', img_src) # 4번답
    cv2.imshow('trap ', img_bit)  # 1번답
    cv2.imshow('whithe, yellow', b) # 1번답
    cv2.imshow('whithe + yellow', img_bit_wy_b) # 2번답
    cv2.imshow('whithe + yellow = src', img_dst) # 3번답

    key = cv2.waitKey(0) # 33ms마다
